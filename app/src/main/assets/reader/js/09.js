...
(()=>{
  ...
  function identityFromVerse(el){if(!el||mode==='hyper'||!CORPORA[mode])return null;const p=pos(),cp=corpus(),b=cp.books[p.b],v=Number(el.dataset.v||String(el.id||'').replace(/^v/,''));if(!Number.isFinite(v))return null;return{book:b.book,chapter:Number(p.c),verse:v,label:bookName(b)+' '+p.c+':'+v,sourceMode:mode}}
  ...
  function positionPopover(el){const r=el.getBoundingClientRect();pop.classList.add('on');pop.setAttribute('aria-hidden','false');requestAnimationFrame(()=>{const h=pop.offsetHeight||60,w=pop.offsetWidth;let top=r.top-h-9;if(top<8)top=Math.min(window.innerHeight-h-8,r.bottom+9);pop.style.top=Math.max(8,top)+'px'})}
  function closePop(){pop.classList.remove('on');pop.setAttribute('aria-hidden','true');activeVerseEl?.classList.remove('verse-context');activeVerseEl=null}
  function openPop(el){closePop();activeVerseEl=el;activeRef=identityFromVerse(el);if(!activeRef)return;el.classList.add('verse-context');positionPopover(el)}
  window.DoxaVerseActions={open:openPop,close:closePop};
  document.addEventListener('touchstart',e=>{if(pop.classList.contains('on')&&!e.target.closest('#verseActions')&&!e.target.closest('.verse-context'))closePop()},{passive:true});window.addEventListener('scroll',()=>{if(pop.classList.contains('on'))closePop()},{passive:true});
