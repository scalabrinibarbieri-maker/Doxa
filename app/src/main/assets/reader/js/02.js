...
/* Doxa 45.1 · A busca compara o texto SEM sinais (vogais, acentos), mas o recorte do trecho e
   o realce usavam essas mesmas posições no texto COM sinais. No português quase não se nota;
   no hebraico, cheio de pontos vocálicos, o realce caía em outra palavra. Agora cada posição
   do texto sem sinais é convertida de volta para a posição real no texto exibido. */
function normIndexMap(flat){const map=[];let i=0;for(const ch of flat){const n=norm(ch);for(let k=0;k<n.length;k++)map.push(i);i+=ch.length}map.push(flat.length);return map}
function searchCorpus(term,m){const out=[],q=norm(term.trim()),cp=CORPORA[m];if(!q)return out;for(let bi=0;bi<cp.books.length;bi++){const b=cp.books[bi];for(const c of b.chapters){for(const v of c.verses){const flat=v.text,hay=norm(flat);let from=0,p,map=null;while((p=hay.indexOf(q,from))!==-1){if(!map)map=normIndexMap(flat);const s0=map[p];let e0=map[p+q.length-1]+1;while(e0<flat.length&&/\p{M}/u.test(flat[e0]))e0++;const a=Math.max(0,s0-55),z=Math.min(flat.length,e0+80);out.push({kind:m,bi,ch:c.chapter,v:v.number,ref:bookName(b)+' '+c.chapter+'.'+v.number,pre:(a>0?'…':'')+flat.slice(a,s0),mid:flat.slice(s0,e0),post:flat.slice(e0,z)+(z<flat.length?'…':'')});from=p+Math.max(1,q.length)}}}}return out}
