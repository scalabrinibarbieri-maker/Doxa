...
Este repositório inicia a migração de APKs modificados para um projeto Android compilado a partir do código-fonte. A primeira etapa contém o aplicativo Android, importação local verificável da V29, integração de arquivos e compilação automática. **O código do leitor V29 ainda está dentro do pacote pessoal importado; sua separação em módulos é a próxima etapa.**
...
5. Aguarde a conferência dos 90 arquivos. O leitor abre e os arquivos ficam disponíveis nas próximas aberturas, offline.
...
Os 90 arquivos do leitor são importados byte a byte: `index.html`, Gênesis 2–50 e Êxodo 1–40. Gênesis 1 permanece dentro do HTML. O código de HUD, seleção, leitura paralela, Strong, menu de estudo e remapeamento de Êxodo permanece intacto.
...
- Para desenvolvimento com SDK instalado: `gradle :app:assembleDebug :app:lintDebug` usando Gradle 8.11.1. Ainda não há Gradle Wrapper neste repositório; o CI instala a versão fixada.
...
- Versionar e separar incrementalmente o código do leitor e seus dados, mantendo a V29 como referência.
